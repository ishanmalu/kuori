...
- Conversion engine with a declarative capability graph (`Sources/Zest/Core/Engine.swift`).
...
- `zest` CLI: `convert`, `formats`, `info` — same path the GUI uses.
- Menu-bar app with a floating Drop Zone; multi-file drops offer the intersection
  of each file's routes. Finder Services entry ("Convert with Zest…").
- `Zest --selftest`: format resolution, graph integrity, argument builders,
